...
import dpdata.formats.deepmd.dpspin_tf_comp
...
class DeePMDCompFormat(Format):
    """DeePMD-kit spin (``dpspin``) npy dataset directory.

    The spin model stores the atomic magnetic moments and the magnetic forces
    of virtual atoms next to the usual coordinates and forces. This format
    maps those arrays back to the ``spins`` and ``force_mags`` data, and writes
    them out again with the requested spin norms and virtual atom lengths.
    """

    def from_system(self, file_name, type_map=None, **kwargs):
        """Load an unlabeled spin dataset.

        Parameters
        ----------
        file_name : str
            Spin dataset directory.
        type_map : list[str], optional
            Element names used when ``type_map.raw`` is absent.
        **kwargs : dict
            ``spin_norm`` and ``virtual_len`` describe how the appended virtual
            atoms encode the spins.

        Returns
        -------
        dict
            System data, including ``spins``.
        """
        return dpdata.formats.deepmd.dpspin_tf_comp.to_system_data(
            file_name, spin_norm=np.array(kwargs["spin_norm"]), virtual_len=np.array(kwargs["virtual_len"]), type_map=type_map, labels=False
        )

    def to_system(self, data, file_name, set_size=5000, prec=np.float64, **kwargs):
        """Dump the system as a spin dataset directory.

        Parameters
        ----------
        data : dict
            The system data, including ``spins`` and ``force_mags``.
        file_name : str
            Output directory.
        set_size : int, default=5000
            Number of frames stored in each ``set.XXX`` directory.
        prec : numpy dtype, default=numpy.float64
            Precision used when the frames are written.
        **kwargs : dict
            ``spin_norm`` and ``virtual_len`` describe how the appended virtual
            atoms encode the spins.
        """
        dpdata.formats.deepmd.dpspin_tf_comp.dump(file_name, data, spin_norm=np.array(kwargs["spin_norm"]), virtual_len=np.array(kwargs["virtual_len"]), set_size=set_size, comp_prec=prec)

    def from_labeled_system(self, file_name, type_map=None, **kwargs):
        """Load a labeled spin dataset.
        ...
        **kwargs : dict
        ...
        -------
        dict
            Labeled system data, including ``spins`` and ``force_mags``.
        """
        return dpdata.formats.deepmd.dpspin_tf_comp.to_system_data(
