        ...
        for dtype in dtypes:
            ...
            if os.path.exists(raw_file):
                ...
                if np.size(tmp) != nframes * int(np.prod(shape, dtype=np.int64)):
                    ...
                    warnings.warn(
                        f"Size of {dtype.deepmd_name}.raw is {np.size(tmp)}, which does not match the shape of data type {dtype.name} ({dtype.shape}). This file is not loaded as {dtype.name}."
                    )
                else:
                    data[dtype.name] = np.reshape(tmp, [nframes, *shape])
        return data
    else:
