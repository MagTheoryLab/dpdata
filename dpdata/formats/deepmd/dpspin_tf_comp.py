...
def to_system_data(folder, spin_norm, virtual_len, type_map=None, labels=True):
    ...
    data["force_mags"] = np.concatenate(all_mag_forces, axis=0)
    # allow custom dtypes
    if labels:
        ...
        for ii in sets:
            ...
            if tmp is not None:
                ...
                if tmp.size != expected:
                    # Several data types may share one deepmd_name; only the data
                    # type whose shape matches the stored array owns it.
                    warnings.warn(
...
def dump(folder, data, spin_norm, virtual_len, set_size=5000, comp_prec=np.float32, remove_sets=True):
    ...
    for atom_type in range(ntypes):
        ...
        if spin_norm[atom_type] != -1 and virtual_len[atom_type] != -1:
            ...
            forces[:, current_spin_index:current_spin_index + n_spins_to_add] = data["force_mags"][:, mask].copy()
